...
- **タスクツリービュー**:サイドバーにタスクの親子構造を表示する。親子関係は事後的に変更可能
...
| TaskTreeViewProvider | サイドバーのTreeView描画、親子関係の変更操作(ドラッグ&ドロップまたはコマンド) |
...
- `taskLog.markDone` / `taskLog.markOpen`:ステータス変更(完了 / 未完了)
