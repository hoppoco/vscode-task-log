...
│   │   ├── taskStore.ts            # タスクの永続化・CRUD
│   │   ├── stallDetector.ts        # 停滞判定ロジック
│   │   ├── summaryGenerator.ts     # 要約テキストの組み立て
│   │   └── jiraClient.ts           # Jira REST APIクライアント
...
| `src/commands/` | `package.json`の`contributes.commands`に対応するハンドラ。基本的に`services`を呼び出すだけの薄い層とし、ロジックを持たせない |
