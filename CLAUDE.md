...
品質チェック(および実機確認が必要な場合はその確認)が完了したら、次の作業に進む前に必ずコミットする。ユーザーに指示されるのを待たず、こちらから完了報告とあわせてコミットを申し出る。
...
品質チェック(および実機確認が必要な場合はその確認)が完了したら、次のラウンド・次の話題に進む前に必ずコミットする。ユーザーから指示されるのを待たず、ラウンド完了の報告とあわせてこちらからコミットを申し出る(明示的に見送る場合を除く)。
...
- セキュリティを考慮したコーディグ(XSS対策、入力バリデーションなど)
